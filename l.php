...
<body>
	...
	<div id="page">
		<div id="header">
			<h1>php中文网 探针</h1>
			<div id="lnmplink">
				for <a href="http://www.php.cn/" target="_blank">php中文网 2017</a>
			</div>
			<div id="lnmplink2">
				not <a href="http://www.php.cn" target="_blank">不想显示 php中文网 探针</a>
			</div>
		</div>
    ...
    <?php echo (false!==eregi("phpinfo",$disFuns))? '<font color="red">×</font>' :"<a href='$phpSelf?act=phpinfo' target='_blank'>PHPINFO</a>";?>
			...
			<tr>
				...
				<tr>
					...
					<td colspan="3"><a href='<?php echo $phpSelf;?>?act=Function'
						target='_blank' class='static'>请点这里查看详细！</a></td>
				</tr>
				<tr>
...
?>
    ...
    ?>
		...
		<form action="<?php echo $_SERVER[PHP_SELF]."#bottom";?>"
			method="post">
			<!--MySQL数据库连接检测-->
			<table width="100%" cellpadding="3" cellspacing="0" align="center">
				<tr>
				...
				<tr>
					<td width="15%"></td>
					<td width="60%">地址：<input type="text" name="host" value="localhost"
						size="10" /> 端口：<input type="text" name="port" value="3306"
						size="10" /> 用户名：<input type="text" name="login" size="10" /> 密码：<input
						type="password" name="password" size="10" />
					</td>
					<td width="25%"><input class="btn" type="submit" name="act"
						value="MySQL检测" /></td>
				</tr>
			</table>
  <?php
...
<!--函数检测-->
			<table width="100%" cellpadding="3" cellspacing="0" align="center">
				...
				<tr>
					<td width="15%"></td>
					<td width="60%">请输入您要检测的函数： <input type="text" name="funName"
						size="50" />
					</td>
					<td width="25%"><input class="btn" type="submit" name="act"
						align="right" value="函数检测" /></td>
				</tr>
  <?php
  if ($_POST['act'] == '函数检测') {
  ...
  ?>
		...
		<div id="footer">
			<p>
				php中文网集成最新的Apache+Nginx+IIS+Lighttpd+PHP+MySQL+phpMyAdmin+SQL-Front+Zend
				Loader，一次性安装，无须配置即可使用，是非常方便、好用的PHP调试环境。该程序绿色小巧简易迷你，有专门的控制面板。总之学习PHP只需一个包。对学习PHP的新手来说，WINDOWS下环境配置是一件很困难的事；对老手来说也是一件烦琐的事。因此无论你是新手还是老手，该程序包都是一个不错的选择。
				本程序纯绿色使用完全免费，自由复制传播。版权所有 <a href="http://www.php.cn/"
					target="_blank">php中文网</a>
			</p>
		</div>
