...
class PMA_Theme_Manager
{
    ...
    function init()
    {
        ...
        if (! $this->getThemeCookie() || ! $this->setActiveTheme($this->getThemeCookie())) {
    ...
    {
        if ($this->_themes_path != trim($GLOBALS['cfg']['ThemePath']) || $this->theme_default != $GLOBALS['cfg']['ThemeDefault']) {
            $this->init();
        } else {
    ...
    function setActiveTheme($theme = null)
    {
        if (! $this->checkTheme($theme)) {
            trigger_error(sprintf(__('Theme %s not found!'), htmlspecialchars($theme)), E_USER_ERROR);
            return false;
        }
    ...
    function loadThemes()
    {
        ...
        if ($handleThemes = opendir($this->getThemesPath())) {
            ...
            trigger_error('phpMyAdmin-ERROR: cannot open themes folder: ' . $this->getThemesPath(), E_USER_WARNING);
            return false;
     ...
     *
     * @param boolean $form
     *            whether enclosed by from tags or not
     * @return string
     */
    function getHtmlSelectBox($form = true)
    {
        ...
        if ($form) {
            $select_box .= '<form name="setTheme" method="post" action="index.php"' . ' target="_parent">';
