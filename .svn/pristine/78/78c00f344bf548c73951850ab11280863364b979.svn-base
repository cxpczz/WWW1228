...
function PMA_transformation_getOptions($option_string)
{
    ...
    if (! strlen($option_string) || ! $transform_options = preg_split('/,/', $option_string)) {
        return $result;
    }
 ...
 * @param string $file
 *            transformation file
 * @param string $html_formatted
 *            whether the description should be formatted as HTML
 *            
 ...
 * @param string $transformation_options
 *            the transformation options of the column
 * @param string $forcedelete
 *            force delete, will erase any existing
