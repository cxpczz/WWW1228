...
function PMA_TRI_handleEditor()
{
     ...
     */
    if (count($errors) || (empty($_REQUEST['editor_process_add']) && empty($_REQUEST['editor_process_edit']) && (! empty($_REQUEST['add_item']) || ! empty($_REQUEST['edit_item'])))) // FIXME: this must be simpler than that
{
 ...
 *
 * @param string $mode
 *            If the editor will be used edit a trigger
 *            or add a new one: 'edit' or 'add'.
 * @param array $item
 *            Data for the trigger returned by
