...
if ($GLOBALS['is_ajax_request'] && $_REQUEST['recent_table']) {
    PMA_ajaxResponse('', true, array(
        'options' => PMA_RecentTable::getInstance()->getHtmlSelectOption()
    ));
}
...
$img_plus = PMA_getImage('b_plus.png', '+', array(
    'id' => 'el%dImg'
));
$img_minus = PMA_getImage('b_minus.png', '-', array(
    'id' => 'el%dImg'
));
...
if ($GLOBALS['cfg']['LeftFrameLight'] && strlen($GLOBALS['db'])) {
    ...
    if ($table_count >= $GLOBALS['cfg']['LeftDisplayTableFilterMinimum']) {
        ?>
        <span id="NavFilter"> <span id="clear_fast_filter"
					title="<?php echo __('Clear'); ?>">X</span> <input type="text"
					class="gray" name="fast_filter" id="fast_filter"
					title="<?php echo __('Filter tables by name'); ?>"
					value="<?php echo __('Filter tables by name'); ?>" />
				</span>
        <?php
    }
    ...
    if (! PMA_is_system_schema($db)) {
        ...
        echo '<ul id="newtable"><li><a target="frame_content" href="tbl_create.php' . PMA_generate_common_url(array(
            'db' => $GLOBALS['db']
        )) . '"  class="' . $class . '" >' . PMA_getImage('b_snewtbl.png', _pgettext('short form', 'Create table'), array(
            'id' => "icon_newtable"
        ...
        foreach ($db_group as $db) {
            ...
            if (count($GLOBALS['pma']->databases) > 1) {
                ...
                <a class="item"
					...
					href="index.php?<?php echo $common_url_query; ?>" target="_parent"
...
echo $GLOBALS['cfg']['DefaultTabDatabase'] . '?' . $common_url_query;
            ...
            if (($GLOBALS['cfg']['Server']['CountTables'] === false) || $db['num_tables']) {
                ...
                $child_visible = (bool) (count($GLOBALS['pma']->databases) === 1 || $db_start == $db['name']);
 ...
 * @param array $tables
 *            array of tables/tablegroups
 * @param boolean $visible
 *            whether the list is visible or not
 * @param string $tab_group_full
 *            full tab group name
 * @param string $table_db
 *            db of this table
 */
...
echo $GLOBALS['cfg']['DefaultTabDatabase'] . '?' . $common_url_query;
            ...
            echo '<a href="' . $href . '" title="' . htmlspecialchars(PMA_getTitleForTarget($GLOBALS['cfg']['DefaultTabTable']) . ': ' . $table['Comment'] . ' (' . PMA_formatNumber($table['Rows'], 0) . ' ' . __('Rows')) . ')"' . ' id="' . htmlspecialchars($table_db . '.' . $table['Name']) . '">' . 
