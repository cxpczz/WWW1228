...
class PMA_Index
{
     ...
     * @param string $table
     *            The tablename
     * @param array $indexes_info
     *            Referenced info array
     * @param array $indexes_data
    ...
    static public function getView($table, $schema, $print_mode = false)
    {
        ...
        foreach ($indexes as $index) {
            ...
            if (! $print_mode) {
                ...
                $r .= '" ' . $row_span . '>' . '    <a href="tbl_indexes.php' . PMA_generate_common_url($this_params) . '">' . PMA_getIcon('b_edit.png', __('Edit')) . '</a>' . '</td>' . "\n";
...
class PMA_Index_Column
{
    ...
    {
        return $as_text ? (! $this->_null || $this->_null == 'NO' ? __('No') : __('Yes')) : $this->_null;
    }
