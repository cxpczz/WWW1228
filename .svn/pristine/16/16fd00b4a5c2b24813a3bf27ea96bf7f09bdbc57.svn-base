...
/**
 * Runs query inside import buffer.
 * This is needed to allow displaying
 * of last SELECT, SHOW or HANDLER results and similar nice stuff.
 *
 * @param string $sql
 *            query to run
 * @param string $full
 *            query to display, this might be commented
 * @param bool $controluser
 *            whether to use control user for queries
 * @access public
 */
...
/**
 * Returns the "Excel" column name (i.e.
 * 1 = "A", 26 = "Z", 27 = "AA", etc.)
 *
 ...
 * @param int $last_cumulative_type
 ...
 * @param
 *            string &$cell String representation of the cell for which a best-fit type is to be determined
 ...
 *        
 * @param
 *            &$table array(string $table_name, array $col_names, array $rows)
 ...
 * @param
 *            array &$analyses Analyses of the tables
 * @param
 *            array &$additional_sql Additional SQL statements to be executed
 * @param array $options
...
function PMA_buildSQL($db_name, &$tables, &$analyses = null, &$additional_sql = null, $options = null)
{
    ...
    $message .= sprintf('<br /><li><a href="%s" title="%s">%s</a> (<a href="%s" title="%s">' . __('Options') . '</a>)</li>', $db_url, sprintf(__('Go to database: %s'), htmlspecialchars(PMA_backquote($db_name))), htmlspecialchars($db_name), $db_ops_url, sprintf(__('Edit settings for %s'), htmlspecialchars(PMA_backquote($db_name))));
    ...
    for ($i = 0; $i < $num_tables; ++ $i) {
        ...
        if (! PMA_Table::isView($db_name, $tables[$i][TBL_NAME])) {
            $message .= sprintf('<li><a href="%s" title="%s">%s</a> (<a href="%s" title="%s">' . __('Structure') . '</a>) (<a href="%s" title="%s">' . __('Options') . '</a>)</li>', $tbl_url, sprintf(__('Go to table: %s'), htmlspecialchars(PMA_backquote($tables[$i][TBL_NAME]))), htmlspecialchars($tables[$i][TBL_NAME]), $tbl_struct_url, sprintf(__('Structure of %s'), htmlspecialchars(PMA_backquote($tables[$i][TBL_NAME]))), $tbl_ops_url, sprintf(__('Edit settings for %s'), htmlspecialchars(PMA_backquote($tables[$i][TBL_NAME]))));
        } else {
            $message .= sprintf('<li><a href="%s" title="%s">%s</a></li>', $tbl_url, sprintf(__('Go to view: %s'), htmlspecialchars(PMA_backquote($tables[$i][TBL_NAME]))), htmlspecialchars($tables[$i][TBL_NAME]));
        }
    }
