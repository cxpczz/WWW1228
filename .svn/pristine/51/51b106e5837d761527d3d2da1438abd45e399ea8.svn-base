...
class PMA_Theme
{
     ...
     * @access public
     * @param
     *            string path to images for this theme
     */
    ...
    function loadCss(&$type)
    {
        ...
        $_css_file = $this->getPath() . '/css/theme_' . $type . '.css.php';
        ...
        if ($type != 'print') {
            ...
            if ((file_exists($_sprites_data_file) && is_readable($_sprites_data_file)) && (file_exists($_sprites_css_file) && is_readable($_sprites_css_file))) {
    ...
    function printPreview()
    {
        ...
        echo '<h2>' . htmlspecialchars($this->getName()) . ' (' . htmlspecialchars($this->getVersion()) . ')</h2>';
        echo '<p>';
        echo '<a target="_top" class="take_theme" ' . 'name="' . htmlspecialchars($this->getId()) . '" ' . 'href="index.php' . PMA_generate_common_url(array(
        ...
        echo '[ <strong>' . __('take it') . '</strong> ]</a>' . '</p>' . '</div>';
    }
    ...
    {
        return PMA_USR_BROWSER_AGENT == 'IE' && PMA_USR_BROWSER_VER >= 6 && PMA_USR_BROWSER_VER <= 8 ? 'filter: none' : '';
    }
     ...
     *            Color of gradient start, hex value without #
     * @param string $end_color
     *            Color of gradient end, hex value without #
     *            
     * @return string CSS code.
     */
