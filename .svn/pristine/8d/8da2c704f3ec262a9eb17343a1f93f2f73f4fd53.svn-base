...
if ($server > 0 || (! $cfg['LeftDisplayServers'] && count($cfg['Servers']) > 1)) {
...
if ($server > 0 && $GLOBALS['cfg']['ShowServerInfo']) {
    ...
    echo '        ' . __('Server charset') . ': ' . '        <span xml:lang="en" dir="ltr">' . '           ' . $mysql_charsets_descriptions[$mysql_charset_map['utf-8']] . "\n" . '           (' . $mysql_charset_map['utf-8'] . ')' . "\n" . '        </span>' . "\n" . '    </li>' . "\n";
    echo '  </ul>';
    echo ' </div>';
}
...
if ($GLOBALS['cfg']['ShowServerInfo'] || $GLOBALS['cfg']['ShowPhpInfo']) {
    ...
    if ($GLOBALS['cfg']['ShowServerInfo']) {
        ...
        if ($server > 0) {
            ...
            if (preg_match('#\d+\.\d+\.\d+#', $client_version_str) && in_array($GLOBALS['cfg']['Server']['extension'], array(
                'mysql',
                'mysqli'
            ))) {
...
if ($cfg['SuhosinDisableWarning'] == false && @ini_get('suhosin.request.max_value_length')) {
    trigger_error(sprintf(__('Server running with Suhosin. Please refer to %sdocumentation%s for possible issues.'), '[a@./Documentation.html#faq1_38@_blank]', '[/a]'), E_USER_WARNING);
}
 ...
 *            displayed text
 * @param string $id
 *            id, used for css styles
 * @param string $url
 *            make item as link with $url as target
 * @param string $mysql_help_page
 *            display a link to MySQL's manual
 * @param string $target
 *            special target for $url
 * @param string $a_id
 *            id for the anchor, used for jQuery to hook in functions
 * @param string $class
 *            class for the li element
