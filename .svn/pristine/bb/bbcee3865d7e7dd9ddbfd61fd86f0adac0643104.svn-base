...
function test_db_connection($extension, $connect_type, $host, $port, $socket, $user, $pass = null, $error_key = 'Server')
{
    ...
    return is_null($error) ? true : array(
        $error_key => $error
    );
}
 ...
 *            error message key: $GLOBALS["strConfig$error_lang_key"]
...
{
    return array(
        $path => test_number($path, $values, false, false, 65535, __('Not a valid port number'))
    );
}
...
{
    return array(
        $path => test_number($path, $values, false, false, PHP_INT_MAX, __('Not a positive number'))
    );
}
...
{
    return array(
        $path => test_number($path, $values, false, true, PHP_INT_MAX, __('Not a non-negative number'))
    );
}
...
function validate_by_regex($path, $values, $regex)
{
    ...
    return array(
        $path => ($result ? '' : __('Incorrect value'))
    );
}
...
function validate_upper_bound($path, $values, $max_value)
{
    ...
    return array(
        $path => ($result ? '' : sprintf(__('Value must be equal or lower than %s'), $max_value))
    );
}
?>
