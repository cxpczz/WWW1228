...
abstract class PMA_GIS_Geometry
{
    ...
    {
        return 'bound = new OpenLayers.Bounds(); bound.extend(new OpenLayers.LonLat(' . $scale_data['minX'] . ', ' . $scale_data['minY'] . ').transform(new OpenLayers.Projection("EPSG:' . $srid . '"), map.getProjectionObject())); bound.extend(new OpenLayers.LonLat(' . $scale_data['maxX'] . ', ' . $scale_data['maxY'] . ').transform(new OpenLayers.Projection("EPSG:' . $srid . '"), map.getProjectionObject()));';
    }

    /**
     * Update the min, max values with the given point set.
     *
     * @param string $point_set
     *            Point set
     * @param array $min_max
     ...
     *
     * @param string $point_set
     *            String of comma sperated points
     * @param array $scale_data
     *            Data related to scaling
    ...
    protected function extractPoints($point_set, $scale_data, $linear = false)
    {
        ...
        foreach ($points as $point) {
            ...
            if (isset($cordinates[0]) && trim($cordinates[0]) != '' && isset($cordinates[1]) && trim($cordinates[1]) != '') {
                if ($scale_data != null) {
     ...
     *            points of a polygon in WKT form
     * @param string $srid
     *            spatial reference id
     *            
