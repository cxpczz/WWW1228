...
/**
 * Minimum inclusion? (i.e.
 * for the stylesheet builder)
 */
if (! defined('PMA_MINIMUM_COMMON')) {
 ...
 // end of the "PMA_SQP_parse()" function
     ...
     *
     * @param
     *            string String of type that we have
     * @param
     *            string String of type that we want
     *            
            ...
            if (isset($current_table_ref) && ($seen_end_of_table_ref || $i == $size - 1) && $subresult != $subresult_empty) {
                for ($tr = 0; $tr <= $current_table_ref; $tr ++) {
                    ...
                    for ($se = 0; $se <= $current_select_expr; $se ++) {
                        if (isset($alias) && strlen($alias) && $subresult['select_expr'][$se]['table_true_name'] == $alias) {
        ...
        for ($i = 0; $i < $size; $i ++) {
            ...
            if ($arr[$i]['type'] == 'alpha_reservedWord') {
                ...
                if (! $seen_reserved_word) {
                    ...
                    if ($first_reserved_word == 'DROP' || $first_reserved_word == 'DELETE' || $first_reserved_word == 'TRUNCATE') {
            ...
            if ($arr[$i]['type'] == 'alpha_reservedWord') {
                ...
                if ($upper_data == 'ON') {
                    if (isset($arr[$i + 1]) && $arr[$i + 1]['type'] == 'alpha_reservedWord') {
                        ...
                        // 'no' as an identifier)
                        ($arr[$i + 2]['type'] == 'alpha_identifier' && strtoupper($arr[$i + 2]['data']) == 'NO'))) {
 ...
 // end of the "PMA_SQP_analyze()" function
     ...
     * @param
     *            array The SQL queries html formatted
     *            
 ...
 // end of the "PMA_SQP_formatHtml_colorize()" function
     ...
     *            string mode
     * @param
     *            integer starting token
     * @param
     *            integer number of tokens to format, -1 = all
     *            
    ...
    function PMA_SQP_formatHtml($arr, $mode = 'color', $start_token = 0, $number_of_tokens = -1)
    {
        ...
        for ($i = $start_token; $i < $number_of_tokens; $i ++) {
            ...
            switch ($typearr[2]) {
                ...
                case 'punct':
                    $before .= ' ';
                    if ($docu && isset($PMA_SQPdata_operators_docs[$arr[$i]['data']]) && ($arr[$i]['data'] != '*' || in_array($arr[$i]['type'], array(
                        'digit_integer',
                        'digit_float',
                        'digit_hex'
                    )))) {
                        $before .= PMA_showMySQLDocu('functions', $PMA_SQPdata_operators_docs[$arr[$i]['data']]['link'], false, $PMA_SQPdata_operators_docs[$arr[$i]['data']]['anchor'], true);
                        $after .= '</a>';
                    }
                ...
                case 'alpha_functionName':
                    ...
                    if ($docu && isset($PMA_SQPdata_functions_docs[$funcname])) {
                        $before .= PMA_showMySQLDocu('functions', $PMA_SQPdata_functions_docs[$funcname]['link'], false, $PMA_SQPdata_functions_docs[$funcname]['anchor'], true);
                        $after .= '</a>';
                    }
                    break;
                case 'alpha_reservedWord':
                    ...
                    switch ($arr[$i]['data']) {
                        ...
                        case 'HANDLER':
                            ...
                            break;
                        default:
                            if ($close_docu_link && in_array($arr[$i]['data'], array(
                                'LIKE',
                                'NOT',
                                'IN',
                                'REGEXP',
