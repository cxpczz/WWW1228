...
//检测API路径
if(!defined('COS_API_PATH'))
    define('COS_API_PATH', dirname(__FILE__));
	
//加载conf.inc.php文件,里面保存着OSS的地址以及用户访问的ID和KEY
require_once COS_API_PATH.DIRECTORY_SEPARATOR.'qcloud'.DIRECTORY_SEPARATOR.'Conf.php';
require_once COS_API_PATH.DIRECTORY_SEPARATOR.'qcloud'.DIRECTORY_SEPARATOR.'Http.php';
require_once COS_API_PATH.DIRECTORY_SEPARATOR.'qcloud'.DIRECTORY_SEPARATOR.'Auth.php';
...
class Cosapi
{
    ...
    public static function upload($srcPath, $bucketName, $dstPath, $bizAttr = null) {
		...
		if(substr($dstPath,0,1)!='/') $dstPath="/".$dstPath;
    ...
    private static function delBase($bucketName, $path) {
		...
		#file_put_contents(GWS_ROOT.'robots.txt',array2string($req),FILE_APPEND);
        return self::sendRequest($req);
