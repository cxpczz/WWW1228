<?php
	...
	function file_icon($file,$type = 'png') {
		...
		if($type == 'png') {
			...
			elseif($ext == 'html' || $ext == 'shtm' || $ext == 'htm' || $ext == 'shtml') $ext = 'html';
			elseif ($ext == 'flv' || $ext == 'swf' || $ext == 'rm'|| $ext == 'mp4' || $ext == 'rmvb') $ext = 'flv';
			else $ext = 'do';
		}
	 ...
	 */
	function initupload($module, $catid,$args, $userid, $groupid = '8', $isadmin = '0',$upload_path=APP_PATH){
		...
		//$upload_path = empty($admin_url) ? (substr($upload_path,-1)!='/'?$upload_path.'/':$upload_path) : 'http://'.$admin_url.'/';
		
		$upload_path = substr($upload_path,-1)!='/'?$upload_path.'/':$upload_path;
		...
		swfu = new SWFUpload({
			flash_url:"'.SITE_PROTOCOL.SITE_URL.'/statics/js/swfupload/swfupload.swf?"+Math.random(),
			upload_url:"'.U('attachment/attachments/swfupload@'.$upload_path.'index.php','dosubmit=1').'",
			...
			file_size_limit:"'.($isadmin? '1024000': $file_size_limit).'",
