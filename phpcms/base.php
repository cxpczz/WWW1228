...
require_once(PC_PATH . DS . 'log4php/Logger.php');
	    ...
	    if (isset($_file[$classname.$m]))
	        return true;
	            ...
	            if (empty($m)) { //模块为空则加载系统的
	                $m	 = 'libs'.DS.'Vendor'.DS.$base;
	            }else { // 加载其他模块的类库
	                $m	 = 'modules'.DS.$m.DS.'classes'.DS.$base;
	            }
	            
	            if (substr($m, -1) != '/') $m    .= '/';
	            $classfile       = $m. $classname . $ext;
	            #echo PC_PATH.$classfile;
	            #exit(PC_PATH.$classfile);
	            if (!class_exists(basename($classname),false) && file_exists(PC_PATH.$classfile)) {
	                require PC_PATH.$classfile;
	            }else {
	                halt("The $classname library cannot be found in the directory of Vendor");
