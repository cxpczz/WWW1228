...
class PMA_Drizzle extends Drizzle
{
    ...
    public function addTcp($host, $port, $user, $password, $db, $options)
    {
        ...
        return $dcon instanceof DrizzleCon ? new PMA_DrizzleCon($dcon) : $dcon;
    }
...
class PMA_DrizzleCon
{
     ...
     * Opens database connection if not already done.
     *
     * @param string $query            
     * @param int $bufferMode
     *            PMA_Drizzle::BUFFER_RESULT, PMA_Drizzle::BUFFER_ROW
     * @param int $fetchMode
     *            PMA_Drizzle::FETCH_ASSOC, PMA_Drizzle::FETCH_NUM or PMA_Drizzle::FETCH_BOTH
    ...
    {
        return $this->lastResult ? $this->lastResult->affectedRows() : false;
    }
 ...
 * Allows for reading result rows as an associative array
...
class PMA_DrizzleResult
{
     ...
     *
     * @param int $fetchMode
     *            fetch mode to use, if none given the default one is used
     * @return array|null
     */
