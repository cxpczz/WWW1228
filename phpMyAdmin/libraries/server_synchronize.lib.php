 ...
 *
 * @param array $trg_tables
 *            array of target database table names,
 * @param array $src_tables
 *            array of source database table names,
 * @param
 *            array &$matching_tables empty array passed by reference to save
 *            names of matching tables,
 * @param
 *            array &$uncommon_source_tables empty array passed by reference to save
 ...
 *            matching tables array containing names
 *            of matching tables,
 * @param
 *            array &$uncommon_target_tables empty array passed by reference to save
 ...
 * @param db_link $src_link
 *            connection established with source server
 * @param db_link $trg_link
 *            connection established with target server
 * @param
 *            array &$matching_table array containing matching table names
 * @param
 *            array &$matching_tables_fields A two dimensional array passed by reference to contain names of fields for each matching table
 * @param
 *            array &$update_array A three dimensional array passed by reference to
 *            contain updates required for each matching table
 * @param
 *            array &$insert_array A three dimensional array passed by reference to
 *            contain inserts required for each matching table
 * @param
 *            array &$delete_array Unused
 * @param
 *            array &$fields_num A two dimensional array passed by reference to
 *            contain number of fields for each matching table
 * @param int $matching_table_index
 *            Index of a table from $matching_table array
 * @param
 *            array &$matching_tables_keys A two dimensional array passed by reference to contain names of keys for each matching table
 */
    ...
    if (isset($matching_table[$matching_table_index])) {
        ...
        for ($j = 0; $j < $source_size; $j ++) {
            ...
            if (isset($source_result_set[$j]) && ($all_keys_match)) {
                ...
                $trg_select_query = "SELECT * FROM " . PMA_backquote($trg_db) . "." . PMA_backquote($matching_table[$matching_table_index]) . " WHERE ";
                ...
                if ($target_result_set) {
                    ...
                    $src_select_query = "SELECT * FROM " . PMA_backquote($src_db) . "." . PMA_backquote($matching_table[$matching_table_index]) . " WHERE ";
 ...
 *
 * @param
 *            array &$delete_array array containing rows that are to be deleted
 * @param array $matching_table
 *            array containing matching table names
 * @param int $matching_table_index
 *            index of a table from $matching_table array
 * @param array $trg_keys
 *            array of target table keys
 * @param array $src_keys
 *            array of source table keys
 * @param string $trg_db
 *            name of target database
 * @param db_link $trg_link
 *            connection established with target server
 ...
 *
 * @param array $source_tables_uncommon
 *            table names that are in source db and not in target db
 * @param string $src_db
 *            name of source database
 * @param mixed $src_link
 *            connection established with source server
 * @param int $index
 *            index of a table from $matching_table array
 * @param
 *            array &$row_count number of rows
 *            
 * @return nothing
 */
 ...
 *
 * @param array $table
 *            Matching tables' names
 * @param array $update_array
 *            A three dimensional array containing field
 *            value updates required for each matching table
 * @param string $src_db
 *            Name of source database
 * @param string $trg_db
 *            Name of target database
 * @param mixed $trg_link
 *            Connection established with target server
 * @param int $matching_table_index
 *            index of matching table in matching_table_array
 ...
 * @param int $matching_table_index
 *            index of matching table in matching_table_array
 * @param array $matching_tables_keys
 *            field names that are keys in the matching table
 * @param array $source_columns
 *            source column information
 * @param
 *            array &$add_column_array column names that are to be added in target table
 * @param array $criteria
 *            criteria like type, null, collation, default etc
 * @param array $target_tables_keys
 *            field names that are keys in the target table
 * @param array $uncommon_tables
 *            table names that are present in source db but not in targt db
 * @param
 *            array &$uncommon_tables_fields field names of the uncommon tables
 * @param array $uncommon_cols
 *            column names that are present in target table and not in source table
 * @param
 *            array &$alter_str_array column names that are to be altered
 * @param
 *            array &$source_indexes column names on which indexes are made in source table
 * @param
 *            array &$target_indexes column names on which indexes are made in target table
 * @param
 *            array &$add_indexes_array column names on which index is to be added in target table
 * @param
 *            array &$alter_indexes_array column names whose indexes are to be altered. Only index name and uniqueness of an index can be changed
 * @param
 *            array &$delete_array rows that are to be deleted
 * @param
 *            array &$update_array rows that are to be updated in target
 * @param bool $display            
 */
 ...
 *            connection established with target server
 * @param
 *            array &$uncommon_tables names of tables present in source but not in target
 * @param int $table_index
 *            index of table in $uncommon_tables array
 * @param
 *            array &$uncommon_tables_fields field names of the uncommon table
 * @param bool $display            
 */
 ...
 *            connection established with target server
 * @param array $uncommon_tables
 *            uncommon table names (table names that are present in source but not in target db)
 * @param int $table_index
 *            index of table in matching_table_array
 * @param array $uncommon_tables_fields
 *            field names of the uncommon table
 * @param bool $display            
 *
 ...
 *            index of table in matching_table_array
 * @param array $target_tables_keys
 *            primary key names of the target tables
 * @param array $delete_array
 *            key values of rows that are to be deleted
 * @param bool $display            
 */
        ...
        if (isset($target_tables_keys[$table_index])) {
            ...
            for ($y = 0; $y < sizeof($target_tables_keys[$table_index]); $y ++) {
                ...
                $pk_query = "SELECT * FROM information_schema.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_SCHEMA = '" . $trg_db . "'
                            AND REFERENCED_TABLE_NAME = '" . $matching_tables[$table_index] . "' AND REFERENCED_COLUMN_NAME = '" . $target_tables_keys[$table_index][$y] . "' AND TABLE_NAME <> REFERENCED_TABLE_NAME;";
 ...
 *            names of matching tables
 * @param
 *            array &$source_columns columns information of the source tables
 * @param
 *            array &$target_columns columns information of the target tables
 * @param
 *            array &$alter_str_array three dimensional associative array first index being the matching table index, second index being column name for which target
 *            column have some criteria different and third index containing the criteria which is different.
 * @param
 *            array &$add_column_array two dimensional associative array, first index of the array contain the matching table number and second index contain the
 *            column name which is to be added in the target table
 * @param
 *            array &$uncommon_columns columns that are present in the target table but not in the source table
 * @param array $criteria
 *            criteria which are to be checked for field that is present in source table and target table
 * @param
 *            array &$target_tables_keys field names which is key in the target table
 * @param int $matching_table_index
 *            number of the matching table
 */
            ...
            for ($i = 0; $i < sizeof($criteria); $i ++) {
                if ($source_columns[$matching_table_index][$column_name][$criteria[$i]] != $target_columns[$matching_table_index][$column_name][$criteria[$i]]) {
                    if (($criteria[$i] == 'Default') && ($source_columns[$matching_table_index][$column_name][$criteria[$i]] == '')) {
                        ...
                        if (! (($criteria[$i] == 'Key') && (($source_columns[$matching_table_index][$column_name][$criteria[$i]] == 'MUL') || ($target_columns[$matching_table_index][$column_name][$criteria[$i]] == 'MUL') || ($source_columns[$matching_table_index][$column_name][$criteria[$i]] == 'UNI') || ($target_columns[$matching_table_index][$column_name][$criteria[$i]] == 'UNI')))) {
 ...
 *            names of matching tables
 * @param array $source_columns
 *            columns information of the source tables
 * @param
 *            array &$add_column_array the names of the column(field) that are to be added in the target
 * @param array $matching_tables_fields            
 * @param array $criteria
 *            criteria
 * @param array $matching_tables_keys
 *            field names which is key in the source table
 * @param array $target_tables_keys
 *            field names which is key in the target table
 * @param array $uncommon_tables
 *            table names that are present in source db and not in target db
 * @param
 *            array &$uncommon_tables_fields names of the fields of the uncommon tables
 * @param int $table_counter
 *            number of the matching table
 * @param array $uncommon_cols            
 * @param bool $display            
 */
function PMA_addColumnsInTargetTable($src_db, $trg_db, $src_link, $trg_link, $matching_tables, $source_columns, &$add_column_array, $matching_tables_fields, $criteria, $matching_tables_keys, $target_tables_keys, $uncommon_tables, &$uncommon_tables_fields, $table_counter, $uncommon_cols, $display)
{
    for ($i = 0; $i < sizeof($matching_tables_fields[$table_counter]); $i ++) {
        if (isset($add_column_array[$table_counter][$matching_tables_fields[$table_counter][$i]])) {
            $query = "ALTER TABLE " . PMA_backquote($trg_db) . '.' . PMA_backquote($matching_tables[$table_counter]) . " ADD COLUMN " . PMA_backquote($add_column_array[$table_counter][$matching_tables_fields[$table_counter][$i]]) . " " . $source_columns[$table_counter][$matching_tables_fields[$table_counter][$i]]['Type'];
            ...
            $is_fk_query = "SELECT * FROM  information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = '" . $trg_db . "' AND TABLE_NAME = '" . $matching_tables[$table_counter] . "' AND COLUMN_NAME ='" . $add_column_array[$table_counter][$matching_tables_fields[$table_counter][$i]] . "' AND TABLE_NAME <> REFERENCED_TABLE_NAME;";
            ...
            if (isset($is_fk_result)) {
                ...
                $fk_query = "ALTER TABLE " . PMA_backquote($trg_db) . '.' . PMA_backquote($matching_tables[$table_counter]) . "ADD CONSTRAINT FOREIGN KEY " . PMA_backquote($add_column_array[$table_counter][$matching_tables_fields[$table_counter][$i]]) . "
                            (" . $add_column_array[$table_counter][$matching_tables_fields[$table_counter][$i]] . ") REFERENCES " . PMA_backquote($trg_db) . '.' . PMA_backquote($is_fk_result[0]['REFERENCED_TABLE_NAME']) . " (" . $is_fk_result[0]['REFERENCED_COLUMN_NAME'] . ");";
 ...
 *            table whose column is a foreign key in another table
 * @param
 *            array &$uncommon_tables names that are uncommon
 * @param
 *            array &$uncommon_tables_fields field names of the uncommon table
 * @param bool $display            
 */
 ...
 *            names of matching tables
 * @param
 *            array &$source_columns columns information of the source table
 * @param
 *            array &$alter_str_array column name and criteria which is to be altered for the targert table
 * @param array $matching_tables_fields
 *            name of the fields for the matching table
 * @param array $criteria
 *            criteria
 * @param
 *            array &$matching_tables_keys field names which is key in the source table
 * @param
 *            array &$target_tables_keys field names which is key in the target table
 * @param int $matching_table_index
 *            number of the matching table
...
function PMA_alterTargetTableStructure($trg_db, $trg_link, $matching_tables, &$source_columns, &$alter_str_array, $matching_tables_fields, $criteria, &$matching_tables_keys, &$target_tables_keys, $matching_table_index, $display)
{
    ...
    for ($t = 0; $t < sizeof($matching_tables_fields[$matching_table_index]); $t ++) {
        if ((isset($alter_str_array[$matching_table_index][$matching_tables_fields[$matching_table_index][$t]])) && (sizeof($alter_str_array[$matching_table_index][$matching_tables_fields[$matching_table_index][$t]]) > 0)) {
            $sql_query = 'ALTER TABLE ' . PMA_backquote($trg_db) . '.' . PMA_backquote($matching_tables[$matching_table_index]) . ' MODIFY ' . PMA_backquote($matching_tables_fields[$matching_table_index][$t]) . ' ' . $source_columns[$matching_table_index][$matching_tables_fields[$matching_table_index][$t]]['Type'];
            $found = false;
            for ($i = 0; $i < sizeof($criteria); $i ++) {
    ...
    for ($p = 0; $p < sizeof($matching_tables_keys[$matching_table_index]); $p ++) {
        if ((isset($alter_str_array[$matching_table_index][$matching_tables_keys[$matching_table_index][$p]]['Key']))) {
            $check = true;
            $query .= ' MODIFY ' . PMA_backquote($matching_tables_keys[$matching_table_index][$p]) . ' ' . $source_columns[$matching_table_index][$matching_tables_fields[$matching_table_index][$p]]['Type'] . ' Not Null ';
 ...
 * @param array $matching_tables
 *            names of matching tables
 * @param array $uncommon_columns
 *            array containing the names of the column which are to be dropped from the target table
 * @param int $table_counter
 *            index of the matching table as in $matchiing_tables array
 * @param bool $display            
 */
function PMA_removeColumnsFromTargetTable($trg_db, $trg_link, $matching_tables, $uncommon_columns, $table_counter, $display)
{
    if (isset($uncommon_columns[$table_counter])) {
        ...
        for ($a = 0; $a < sizeof($uncommon_columns[$table_counter]); $a ++) {
            ...
            $pk_query = "SELECT * FROM information_schema.KEY_COLUMN_USAGE WHERE REFERENCED_TABLE_SCHEMA = '" . $trg_db . "'
                         AND REFERENCED_TABLE_NAME = '" . $matching_tables[$table_counter] . "' AND REFERENCED_COLUMN_NAME = '" . $uncommon_columns[$table_counter][$a] . "' AND TABLE_NAME <> REFERENCED_TABLE_NAME;";
            ...
            $query = "SELECT * FROM information_schema.KEY_COLUMN_USAGE WHERE TABLE_SCHEMA = '" . $trg_db . "' AND TABLE_NAME = '" . $matching_tables[$table_counter] . "' AND COLUMN_NAME = '" . $uncommon_columns[$table_counter][$a] . "'
 ...
 *            connection established with target server
 * @param array $matching_tables
 *            matching tables name
 * @param
 *            array &$source_indexes indexes of the source table
 * @param
 *            array &$target_indexes indexes of the target table
 * @param
 *            array &$add_indexes_array name of the column on which the index is to be added in the target table
 * @param
 *            array &$alter_indexes_array key name which needs to be altered
 * @param
 *            array &$remove_indexes_array key name of the index which is to be removed from the target table
 * @param int $table_counter
 *            number of the matching table
 */
 ...
 * @param array $matching_tables
 *            matching tables name
 * @param array $source_indexes
 *            indexes of the source table
 * @param array $target_indexes
 *            indexes of the target table
 * @param array $add_indexes_array
 *            column names on which indexes are to be created in target table
 * @param array $alter_indexes_array
 *            column names for which indexes are to be altered
 * @param array $remove_indexes_array
 *            key name of the indexes which are to be removed from the target table
 * @param int $table_counter
 *            number of the matching table
 * @param
 *            $display
 ...
 * @param string $column
 *            name of the column to retrieve data from
 * @param mixed $link
 *            mysql link resource
 *            
