...
function PMA_EVN_handleEditor()
{
                ...
                if (! $result) {
                    ...
                    if (! $result) {
                        ...
                        if (! $result) {
                            ...
                            $errors[] = __('Sorry, we failed to restore the dropped event.') . '<br />' . __('The backed up query was:') . "\"" . htmlspecialchars($create_item) . "\"" . '<br />' . __('MySQL said: ') . PMA_DBI_getError(null);
                        }
                    } else {
     ...
     */
    if (count($errors) || (empty($_REQUEST['editor_process_add']) && empty($_REQUEST['editor_process_edit']) && (! empty($_REQUEST['add_item']) || ! empty($_REQUEST['edit_item']) || ! empty($_REQUEST['item_changetype'])))) { // FIXME: this must be simpler than that
        $operation = '';
 ...
 *
 * @param string $mode
 *            If the editor will be used edit an event
 ...
 * @param array $item
 *            Data for the event returned by
