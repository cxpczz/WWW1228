...
function PMA_RTN_main()
{
    ...
    if ($GLOBALS['cfg']['Server']['extension'] === 'mysql') {
        trigger_error(__('You are using PHP\'s deprecated \'mysql\' extension, ' . 'which is not capable of handling multi queries. ' . '<b>The execution of some stored routines may fail!</b> ' . 'Please use the improved \'mysqli\' extension to ' . 'avoid any problems.'), E_USER_WARNING);
    }
}
 // end PMA_RTN_main()
 ...
 *
 * @param string $value
 *            A string containing one parameter of a routine
 *            
...
function PMA_RTN_parseOneParameter($value)
{
    ...
    for ($i = $pos; $i < $parsed_param['len']; $i ++) {
        if (($parsed_param[$i]['type'] == 'alpha_columnType' || $parsed_param[$i]['type'] == 'alpha_functionName') && $depth == 0) // "CHAR" seems to be mistaken for a function by the parser
{
 ...
 *            Parsed query, returned by by PMA_SQP_parse()
...
function PMA_RTN_parseAllParameters($parsed_query, $routine_type)
{
    ...
    foreach ($params as $key => $value) {
        list ($retval['dir'][], $retval['name'][], $retval['type'][], $retval['length'][], $retval['opts'][]) = PMA_RTN_parseOneParameter($value);
    }
 ...
 *
 * @param array $parsed_query
 *            Parsed query, returned by PMA_SQP_parse()
 *            
...
function PMA_RTN_handleEditor()
{
    ...
    if (! empty($_REQUEST['editor_process_add']) || ! empty($_REQUEST['editor_process_edit'])) {
        ...
        if (! count($errors)) { // set by PMA_RTN_getQueryFromRequest()
            ...
            if (! empty($_REQUEST['editor_process_edit'])) {
                if (! in_array($_REQUEST['item_original_type'], array(
                    'PROCEDURE',
                    'FUNCTION'
                ))) {
        ...
        if ($GLOBALS['is_ajax_request']) {
            ...
            if ($message->isSuccess()) {
                ...
                $where = "ROUTINE_SCHEMA='" . PMA_sqlAddSlashes($db) . "' " . "AND ROUTINE_NAME='" . PMA_sqlAddSlashes($_REQUEST['item_name']) . "'" . "AND ROUTINE_TYPE='" . PMA_sqlAddSlashes($_REQUEST['item_type']) . "'";
     ...
     */
    if (count($errors) || (empty($_REQUEST['editor_process_add']) && empty($_REQUEST['editor_process_edit']) && (! empty($_REQUEST['add_item']) || ! empty($_REQUEST['edit_item']) || ! empty($_REQUEST['routine_addparameter']) || ! empty($_REQUEST['routine_removeparameter']) || ! empty($_REQUEST['routine_changetype'])))) // FIXME: this must be simpler than that
{
 ...
 * complete the editor form.
 * It is especially necessary to handle
...
function PMA_RTN_getDataFromRequest()
{
    ...
    if (isset($_REQUEST['item_original_type']) && $_REQUEST['item_original_type'] == 'FUNCTION') {
    ...
    if (isset($_REQUEST['item_param_name']) && isset($_REQUEST['item_param_type']) && isset($_REQUEST['item_param_length']) && isset($_REQUEST['item_param_opts_num']) && isset($_REQUEST['item_param_opts_text']) && is_array($_REQUEST['item_param_name']) && is_array($_REQUEST['item_param_type']) && is_array($_REQUEST['item_param_length']) && is_array($_REQUEST['item_param_opts_num']) && is_array($_REQUEST['item_param_opts_text'])) {
        ...
        $retval['item_num_params'] = max(count($retval['item_param_name']), count($retval['item_param_type']), count($retval['item_param_length']), count($retval['item_param_opts_num']), count($retval['item_param_opts_text']));
    }
    $retval['item_returntype'] = '';
    if (isset($_REQUEST['item_returntype']) && in_array($_REQUEST['item_returntype'], PMA_getSupportedDatatypes())) {
        $retval['item_returntype'] = $_REQUEST['item_returntype'];
    }
    
    $retval['item_isdeterministic'] = '';
    if (isset($_REQUEST['item_isdeterministic']) && strtolower($_REQUEST['item_isdeterministic']) == 'on') {
    ...
    if (isset($_REQUEST['item_sqldataaccess']) && in_array($_REQUEST['item_sqldataaccess'], $param_sqldataaccess, true)) {
 ...
 * @param bool $all
 *            Whether to return all data or just
...
function PMA_RTN_getDataFromName($name, $type, $all = true)
{
    ...
    $where = "ROUTINE_SCHEMA='" . PMA_sqlAddSlashes($db) . "' " . "AND SPECIFIC_NAME='" . PMA_sqlAddSlashes($name) . "'" . "AND ROUTINE_TYPE='" . PMA_sqlAddSlashes($type) . "'";
 ...
 *
 * @param array $routine
 *            Data for the routine returned by
 *            PMA_RTN_getDataFromRequest() or
 *            PMA_RTN_getDataFromName()
 * @param mixed $index
 *            Either a numeric index of the row being processed
 *            or NULL to create a template row for AJAX request
 * @param string $class
 *            Class used to hide the direction column, if the
...
function PMA_RTN_getParameterRow($routine = array(), $index = null, $class = '')
{
    ...
    $retval .= "                        " . PMA_getImage('b_edit', '', array(
        'title' => __('ENUM/SET editor')
    )) . "\n";
    $retval .= "                    </a>\n";
    $retval .= "                </div>\n";
    $retval .= "            </td>\n";
 ...
 *
 * @param string $mode
 *            If the editor will be used edit a routine
 *            or add a new one: 'edit' or 'add'.
 * @param string $operation
 *            If the editor was previously invoked with
...
function PMA_RTN_getExecuteForm($routine)
{
    ...
    for ($i = 0; $i < $routine['item_num_params']; $i ++) { // Each parameter
        ...
        if (in_array($routine['item_param_type'][$i], array(
            'ENUM',
            'SET'
        ))) {
