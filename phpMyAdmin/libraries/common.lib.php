 ...
 * @param string $exp
 *            exponent to use
 * @param mixed $use_function
 *            pow function to use, or false for auto-detect
 *            
 ...
 *            the string to slash
 * @param bool $is_like
 *            whether the string will be used in a 'LIKE' clause
 *            (it then requires two more escaped sequences) or not
 * @param bool $crlf
 *            whether to treat cr/lfs as escape-worthy entities
 *            (converts \n to \\n, \r to \\r)
 * @param bool $php_code
 *            whether this function is used as part of the
...
function PMA_unQuote($quoted_string, $quote = null)
{
    ...
    foreach ($quotes as $quote) {
        if (substr($quoted_string, 0, 1) === $quote && substr($quoted_string, - 1, 1) === $quote) {
 ...
 * format sql strings
 *
 * @param mixed $parsed_sql
 *            pre-parsed SQL structure
 ...
 * @param string $anchor
 *            anchor to page part
 * @param bool $just_open
 *            whether only the opening <a> tag should be returned
 *            
...
function PMA_showDocu($anchor)
{
    if ($GLOBALS['cfg']['ReplaceHelpImg']) {
        return '<a href="Documentation.html#' . $anchor . '" target="documentation">' . PMA_getImage('b_help.png', __('Documentation')) . '</a>';
    } else {
...
function PMA_showHint($message, $bbcode = false, $type = 'notice')
{
    ...
    return '<sup class="footnotemarker">' . $nr . '</sup>' . PMA_getImage('b_help.png', '', array(
        'class' => 'footnotemarker footnote_' . $nr
    ));
}
 ...
 * @param bool $is_modify_link
 *            whether to show a "modify" link or not
 * @param string $back_url
 *            the "back" link url (full path is not required)
 * @param bool $exit
 *            EXIT the page?
 *            
...
function PMA_getTableList($db, $tables = null, $limit_offset = 0, $limit_count = false)
{
    ...
    foreach ($tables as $table_name => $table) {
        ...
        if ($GLOBALS['cfg']['ShowTooltipAliasTB'] && $GLOBALS['cfg']['ShowTooltipAliasTB'] !== 'nested' && $table['Comment'] && // do not switch if the comment is empty
 ...
 *            or array of it
 * @param boolean $do_it
 *            a flag to bypass this function (used by dump
 *            functions)
 *            
 ...
 * @param string $type
 *            the type (level) of the message
 * @param boolean $is_view
 *            is this a message after a VIEW operation?
 *            
 * @return string
 *
 * @access public
...
function PMA_showMessage($message, $sql_query = null, $type = 'notice', $is_view = false)
{
    ...
    if ($cfg['ShowSQL'] == true && ! empty($sql_query)) {
        ...
        if (! empty($cfg['SQLQuery']['Edit'])) {
            ...
            $edit_link = ' [' . PMA_linkOrButton($edit_link, __('Edit'), array(
                'onclick' => $onclick
            )) . ']';
        } else {
            $edit_link = '';
        ...
        // Refresh query
        if (! empty($cfg['SQLQuery']['Refresh']) && ! isset($GLOBALS['show_as_php']) && // 'Submit query' does the same
 ...
 * @param double $value
 *            the value to format
 * @param integer $digits_left
 *            number of digits left of the comma
 * @param integer $digits_right
 *            number of digits right of the comma
 * @param boolean $only_down
 *            do not reformat numbers below 1
 * @param boolean $noTrailingZero
 *            removes trailing zeros right of the comma
 *            (default: true)
 *            
...
{
    $tab_navigation = '<div id="' . htmlentities($menu_id) . 'container" class="menucontainer">' . '<ul id="' . htmlentities($menu_id) . '">';
 ...
 * @param string $message
 *            the link message
 * @param mixed $tag_params
 *            string: js confirmation
 *            array: additional tag params (f.e. style="")
 * @param boolean $new_form
 *            we set this to false when we are already in
 *            a form, to avoid generating nested forms
 * @param boolean $strip_img
 *            whether to strip the image
 * @param string $target
 *            target
 *            
...
/**
 * Takes a string and outputs each character on a line for itself.
 * Used
 ...
 *
 * @param array $params
 *            The names of the parameters needed by the calling script.
 * @param bool $die
 *            Stop the execution?
 *            (Set this manually to false in the calling script
 *            until you know all needed parameters to check).
 * @param bool $request
 *            Whether to include this list in checking for special params.
 *            
 ...
 * @param integer $fields_cnt
 *            number of fields
 * @param array $fields_meta
...
function PMA_getUniqueCondition($handle, $fields_cnt, $fields_meta, $row, $force_unique = false)
{
    ...
    for ($i = 0; $i < $fields_cnt; ++ $i) {
        ...
        if (! isset($row[$i]) || is_null($row[$i])) {
            ...
            stristr($field_flags, 'BINARY') && ! empty($row[$i])) {
 ...
 *            name of button element
 * @param string $button_class
 *            class of button element
 * @param string $image_name
 *            name of image element
 * @param string $text
 *            text to display
 * @param string $image
 *            image to display
...
function PMA_buttonOrImage($button_name, $button_class, $image_name, $text, $image, $value = '')
{
    ...
    if (false === $GLOBALS['cfg']['PropertiesIconic']) {
        echo ' <input type="submit" name="' . $button_name . '"' . ' value="' . htmlspecialchars($value) . '"' . ' title="' . htmlspecialchars($text) . '" />' . "\n";
        return;
    }
    ...
    if (PMA_USR_BROWSER_AGENT != 'IE') {
        echo '<button class="' . $button_class . '" type="submit"' . ' name="' . $button_name . '" value="' . htmlspecialchars($value) . '"' . ' title="' . htmlspecialchars($text) . '">' . "\n" . PMA_getIcon($image, $text) . '</button>' . "\n";
    } else {
        echo '<input type="image" name="' . $image_name . '" value="' . htmlspecialchars($value) . '" title="' . htmlspecialchars($text) . '" src="' . $GLOBALS['pmaThemeImage'] . $image . '" />' . ($GLOBALS['cfg']['PropertiesIconic'] === 'both' ? '&nbsp;' . htmlspecialchars($text) : '') . "\n";
    }
}
 // end function
 ...
 * @param int $pageNow
 *            current page number
 * @param int $nbTotalPage
 *            number of total pages
 * @param int $showAll
 *            If the number of pages is lower than this
 ...
 * @param int $percent
 *            Percentage of calculation page offsets to hop to a
 *            next page
 * @param int $range
 *            Near the current page, how many pages should
 ...
 *            localized message explaining the func.
 * @param string $component
 *            'mysql' (eventually, 'php')
 * @param string $minimum_version
 *            of this component
 * @param string $bugref
 *            bug reference for this component
 */
 ...
 *
 * @param string $html_field_name
 *            the checkbox HTML field
 * @param string $label
 *            label for checkbox
 * @param boolean $checked
 *            is it initially checked?
 * @param boolean $onclick
 *            should it submit the form on click?
 *            
 ...
 *
 * @param string $html_field_name
 *            the radio HTML field
 * @param array $choices
 *            the choices values and labels
 * @param string $checked_choice
 *            the choice to check by default
 * @param boolean $line_break
 *            whether to add an HTML line break after a choice
 * @param boolean $escape_label
 *            whether to use htmlspecialchars() on label
 * @param string $class
 *            enclose each choice with a div of this class
 *            
 ...
 * @param array $choices
 *            choices values
 * @param string $active_choice
 *            the choice to select by default
 * @param string $id
 *            id of the select element; can be different in case
 ...
 *
 * @param string $id
 *            the id of the <div> on which to apply the effect
 * @param string $message
 *            the message to show as a link
 */
function PMA_generate_slider_effect($id, $message)
{
    ...
    /**
     * Bad hack on the next line.
     * document.write() conflicts with jQuery, hence,
    ...
    ?>
<div id="<?php echo $id; ?>"
	<?php echo $GLOBALS['cfg']['InitialSlidersState'] == 'closed' ? ' style="display: none; overflow:auto;"' : ''; ?>
 ...
 *
 * @param string $action
 *            The URL for the request to be executed
 * @param string $select_name
 *            The name for the dropdown box
 * @param array $options
 *            An array of options (see rte_footer.lib.php)
 * @param string $callback
 *            A JS snippet to execute when the request is
...
{
    return strtr($bit_default_value, array(
        "b" => "",
        "'" => ""
    ));
}
 ...
 *
 * @param string $target
 *            a valid value for $cfg['LeftDefaultTabTable'],
 ...
 * @param array $updates
 *            Array with overrides for default parameters
 ...
 * execution
 *
 * @param PMA_Message|string $message
 *            message string containing the
 *            html of the message
 * @param bool $success
 *            success whether the ajax request
 *            was successfull
 ...
 *
 * @param bool $html
 *            Whether to generate an html snippet or an array
 ...
 *            if provided the output is limited to the functions
 *            that are applicable to the provided geometry type.
 * @param bool $binary
 *            if set to false functions that take two geometries
 *            as arguments will not be included.
 * @param bool $display
 *            if set to true seperators will be added to the
 *            output array.
 *            
...
function PMA_getGISFunctions($geom_type = null, $binary = true, $display = false)
{
    ...
    $funcs['Dimension'] = array(
        'params' => 1,
        'type' => 'int'
    );
    $funcs['Envelope'] = array(
        'params' => 1,
        'type' => 'Polygon'
    );
    $funcs['GeometryType'] = array(
        'params' => 1,
        'type' => 'text'
    );
    $funcs['SRID'] = array(
    ...
    if ($geom_type == 'point') {
        ...
        $funcs['NumPoints'] = array(
            'params' => 1,
            'type' => 'int'
        );
        $funcs['StartPoint'] = array(
            'params' => 1,
            'type' => 'point'
        );
        $funcs['IsRing'] = array(
        ...
        $funcs['ExteriorRing'] = array(
            'params' => 1,
            'type' => 'linestring'
        );
        $funcs['NumInteriorRings'] = array(
            'params' => 1,
            'type' => 'int'
        );
    ...
    if ($binary) {
        ...
        if (PMA_MYSQL_INT_VERSION < 50601) {
            ...
            $funcs['Contains'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['Disjoint'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['Equals'] = array(
            ...
            $funcs['Intersects'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['Overlaps'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['Touches'] = array(
            ...
            $funcs['ST_Crosses'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['ST_Contains'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['ST_Disjoint'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['ST_Equals'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['ST_Intersects'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['ST_Overlaps'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['ST_Touches'] = array(
                'params' => 2,
                'type' => 'int'
            );
            $funcs['ST_Within'] = array(
                'params' => 2,
                'type' => 'int'
            );
        }
        ...
        $funcs['MBRContains'] = array(
            'params' => 2,
            'type' => 'int'
        );
        $funcs['MBRDisjoint'] = array(
            'params' => 2,
            'type' => 'int'
        );
        $funcs['MBREquals'] = array(
            'params' => 2,
            'type' => 'int'
        );
        $funcs['MBRIntersects'] = array(
            'params' => 2,
            'type' => 'int'
        );
        $funcs['MBROverlaps'] = array(
            'params' => 2,
            'type' => 'int'
        );
        $funcs['MBRTouches'] = array(
            'params' => 2,
            'type' => 'int'
        );
        $funcs['MBRWithin'] = array(
            'params' => 2,
            'type' => 'int'
        );
    }
 ...
 *
 * @param array $field
 *            Data about the column for which
 *            to generate the dropdown
 * @param bool $insert_mode
...
function PMA_getFunctionsForField($field, $insert_mode)
{
    ...
    if (isset($cfg['RestrictColumnTypes'][strtoupper($field['True_Type'])]) && isset($cfg['RestrictFunctions'][$cfg['RestrictColumnTypes'][strtoupper($field['True_Type'])]])) {
    ...
    if ($field['True_Type'] == 'timestamp' && empty($field['Default']) && empty($data) && ! isset($analyzed_sql[0]['create_table_fields'][$field['Field']]['on_update_current_timestamp']) && $analyzed_sql[0]['create_table_fields'][$field['Field']]['default_value'] != 'NULL') {
    ...
    for ($j = 0; $j < $cnt_functions; $j ++) {
        if (! isset($dropdown_built[$cfg['Functions'][$j]]) || $dropdown_built[$cfg['Functions'][$j]] != 'true') {
            ...
            $selected = ($field['first_timestamp'] && $cfg['Functions'][$j] == $cfg['DefaultFunctions']['first_timestamp']) || (! $field['first_timestamp'] && $cfg['Functions'][$j] == $default_function) ? ' selected="selected"' : '';
 ...
 * user indeed has that privilege or false if (s)he doesn't.
 * This function must
 ...
 * @param mixed $db
 *            null, to only check global privileges
 *            string, db name where to also check for privileges
 * @param mixed $tbl
 *            null, to only check global/db privileges
 ...
 *            Whether to escape html entitites
 *            
 * @return array
 */
