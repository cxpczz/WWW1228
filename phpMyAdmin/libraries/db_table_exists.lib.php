...
if (empty($is_db)) {
    ...
    if (! $is_db) {
        ...
        if (! defined('IS_TRANSFORMATION_WRAPPER')) {
            ...
            PMA_sendHeaderLocation($cfg['PmaAbsoluteUri'] . 'main.php' . PMA_generate_common_url($url_params, '&'));
        }
        exit();
    }
...
if (empty($is_table) && ! defined('PMA_SUBMIT_MULT') && ! defined('TABLE_MAY_BE_ABSENT')) {
    ...
    if (strlen($table)) {
        ...
        if (! $is_table) {
            $_result = PMA_DBI_try_query('SHOW TABLES LIKE \'' . PMA_sqlAddSlashes($table, true) . '\';', null, PMA_DBI_QUERY_STORE);
    ...
    if (! $is_table) {
        if (! defined('IS_TRANSFORMATION_WRAPPER')) {
            if (strlen($table)) {
                 ...
                 */
                $_result = PMA_DBI_try_query('SELECT COUNT(*) FROM ' . PMA_backquote($table) . ';', null, PMA_DBI_QUERY_STORE);
