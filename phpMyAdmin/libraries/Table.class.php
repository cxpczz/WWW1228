...
class PMA_Table
{
    ...
    static public function isView($db = null, $table = null)
    {
        ...
        $result = PMA_DBI_fetch_result("SELECT TABLE_NAME
    ...
    {
        return PMA_Table::sGetStatusInfo($db, $table, 'Comment') . ' (' . PMA_Table::countRecords($db, $table) . ')';
    }
     ...
     *            read new rather than serving from cache
     * @param boolean $disable_error
     *            if true, disables error message
     *            
     ...
     *            field comment
     * @param
     *            array &$field_primary list of fields for PRIMARY KEY
     * @param string $index            
     *
 ...
 // end function
     ...
     *
     * @param string $work
     *            The array index, which Relation feature to check
     *            ('relwork', 'commwork', ...)
     * @param string $pma_table
     *            The array index, which PMA-table to update
     *            ('bookmark', 'relation', ...)
     * @param array $get_fields
     *            Which fields will be SELECT'ed from the old entry
     * @param array $where_fields
     *            Which fields will be used for the WHERE query
     *            (array('FIELDNAME' => 'FIELDVALUE'))
     * @param array $new_fields
     *            Which fields will be used as new VALUES. These are
        ...
        if (isset($GLOBALS['cfgRelation']) && $GLOBALS['cfgRelation'][$work]) {
            ...
            $table_copy_query = '
                SELECT ' . implode(', ', $select_parts) . '
                  FROM ' . PMA_backquote($GLOBALS['cfgRelation']['db']) . '.' . PMA_backquote($GLOBALS['cfgRelation'][$pma_table]) . '
            ...
            while ($table_copy_row = @PMA_DBI_fetch_assoc($table_copy_rs)) {
                ...
                $new_table_query = 'INSERT IGNORE INTO ' . PMA_backquote($GLOBALS['cfgRelation']['db']) . '.' . PMA_backquote($GLOBALS['cfgRelation'][$pma_table]) . '
 ...
 // end of 'PMA_Table::duplicateInfo()' function
    ...
    public function getUniqueColumns($backquoted = true)
    {
        ...
        $uniques = PMA_DBI_fetch_result($sql, array(
            'Key_name',
            null
        ), 'Column_name');
    ...
    protected function loadUiPrefs()
    {
        ...
        if (! isset($_SESSION['tmp_user_values']['table_uiprefs'][$server_id][$this->db_name][$this->name])) {
            ...
            (strlen($GLOBALS['cfg']['Server']['pmadb']) && strlen($GLOBALS['cfg']['Server']['table_uiprefs'])) ? $this->getUiPrefsFromDb() : array();
        }
    ...
    public function getUiProp($property)
    {
        ...
        } elseif ($property == self::PROP_COLUMN_ORDER || $property == self::PROP_COLUMN_VISIB) {
     ...
     * @param string $table_create_time
