...
class PMA_Schema_PDF extends PMA_PDF
{
    ...
    function Header()
    {
        ...
        if ($with_doc) {
            $test_query = 'SELECT * FROM ' . PMA_backquote($GLOBALS['cfgRelation']['db']) . '.' . PMA_backquote($cfgRelation['pdf_pages']) . ' WHERE db_name = \'' . PMA_sqlAddSlashes($db) . '\'' . ' AND page_nr = \'' . $pdf_page_number . '\'';
...
class Table_Stats
{
     ...
     *            The current page number (from the
     *            $cfg['Servers'][$i]['table_coords'] table)
     * @param
     *            integer &$sameWideWidth The max. with among tables
     * @param boolean $showKeys
...
class PMA_Pdf_Relation_Schema extends PMA_Export_Relation_Schema
{
    ...
    function __construct()
    {
        ...
        $this->scale = ceil(max(($this->_xMax - $this->_xMin) / ($pdf->getPageWidth() - $this->rightMargin - $this->leftMargin), ($this->_yMax - $this->_yMin) / ($pdf->getPageHeight() - $this->topMargin - $this->bottomMargin)) * 100) / 100;
     ...
     *
     * @param string $table
     *            The table name of which sets XY co-ordinates
     *            
     * @return nothing
     *
     * @access private
