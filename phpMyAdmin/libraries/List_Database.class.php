...
class PMA_List_Database extends PMA_List
{
     ...
     * Constructor
     *
     * @param mixed $db_link_user
     *            user database link resource|object
     * @param mixed $db_link_control
     *            control database link resource|object
     */
     ...
     * @param string $like_db_name
     *            usally a db_name containing wildcards
     * @return array
     */
    ...
    public function getHtmlListGrouped($selected = '', $offset, $count)
    {
        ...
        foreach ($this->getGroupedDetails($offset, $count) as $group => $dbs) {
            ...
            foreach ($dbs as $db) {
                ...
                $return .= ' href="index.php?' . PMA_generate_common_url($db['name']) . '" target="_parent">';
                if ($cut) {
    ...
    public function getHtmlSelectGrouped($selected = '', $offset, $count)
    {
        ...
        foreach ($this->getGroupedDetails($offset, $count) as $group => $dbs) {
            ...
            foreach ($dbs as $db) {
                $return .= '<option value="' . htmlspecialchars($db['name']) . '"' . ' title="' . htmlspecialchars($db['comment']) . '"';
