...
class ConfigFile
{
    ...
    public function set($path, $value, $canonical_path = null)
    {
        ...
        if (! isset($this->persistKeys[$canonical_path])) {
            ...
            if (($value === $default_value && (defined('PMA_SETUP') || $instance_default_value === $default_value)) || (empty($value) && empty($default_value) && (defined('PMA_SETUP') || empty($current_global)))) {
    ...
    /**
     * Flattens multidimensional array, changes indices to paths (eg.
     * 'key/subkey').
    ...
    /**
     * Returns default config value or $default it it's not set ie.
     * it doesn't
    ...
    public function removeServer($server)
    {
        ...
        if (isset($_SESSION[$this->id]['ServerDefault']) && $_SESSION[$this->id]['ServerDefault'] >= 0) {
    ...
    {
        $this->_flattenArrayResult = array();
        array_walk($_SESSION[$this->id], array(
            $this,
            '_flattenArray'
        ), '');
