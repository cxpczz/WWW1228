...
class PMA_Config
{
    ...
    function checkOutputCompression()
    {
        ...
        if (strtolower($this->get('OBGzip')) == 'auto') {
            if ($this->get('PMA_USR_BROWSER_AGENT') == 'IE' && $this->get('PMA_USR_BROWSER_VER') >= 6 && $this->get('PMA_USR_BROWSER_VER') < 7) {
                $this->set('OBGzip', false);
            } else {
    ...
    function checkClient()
    {
        ...
        if (preg_match('@Opera(/| )([0-9].[0-9]{1,2})@', $HTTP_USER_AGENT, $log_version)) {
            ...
            // newer Safari
        } elseif (preg_match('@Mozilla/([0-9].[0-9]{1,2})@', $HTTP_USER_AGENT, $log_version) && preg_match('@Version/(.*) Safari@', $HTTP_USER_AGENT, $log_version2)) {
    ...
    function checkWebServer()
    {
        ...
        stristr(PMA_getenv('SERVER_SOFTWARE'), 'Microsoft') && stristr(PMA_getenv('SERVER_SOFTWARE'), 'IIS')) {
    ...
    function checkConfigSource()
    {
        ...
        if (! is_readable($this->getSource())) {
            $this->source_mtime = 0;
            die(sprintf(function_exists('__') ? __('Existing configuration file (%s) is not readable.') : 'Existing configuration file (%s) is not readable.', $this->getSource()));
        }
    ...
    function checkPmaAbsoluteUri()
    {
        ...
        if (strlen($pma_absolute_uri) < 5) {
            ...
            // this.
            if (substr($pma_absolute_uri, 0, 7) != 'http://' && substr($pma_absolute_uri, 0, 8) != 'https://') {
                $pma_absolute_uri = ($is_https ? 'https' : 'http') . ':' . (substr($pma_absolute_uri, 0, 2) == '//' ? '' : '//') . $pma_absolute_uri;
            }
        }
     ...
     * @static
     *
     * @param string $current_size
     *            current selected font size with unit
     *            
     ...
     *
     * @param string $current_size
     *            currently slected font size with unit
     *            
