 ...
 *
 * @param
 *            string &$the_disp_mode the synthetic value for display_mode (see a few
 *            lines above for explanations)
 * @param
 *            integer &$the_total the total number of rows returned by the SQL query
 ...
 *            iconic caption for button
 * @param string $title
 *            text for button
 * @param integer $pos
 *            position for next query
 * @param string $html_sql_query
 *            query ready for display
 * @param string $onsubmit
 *            optional onsubmit clause
 * @param string $input_for_real_end
 *            optional hidden field for special treatment
 * @param string $onclick
 *            optional onclick clause
 *            
 * @return nothing
 *
...
<td>
	<form action="sql.php" method="post" <?php echo $onsubmit; ?>>
        ...
        <input type="hidden" name="sql_query"
			value="<?php echo $html_sql_query; ?>" /> <input type="hidden"
			name="pos" value="<?php echo $pos; ?>" /> <input type="hidden"
			name="goto" value="<?php echo $goto; ?>" />
        <?php echo $input_for_real_end; ?>
        <input type="submit" name="navig"
			<?php echo ($GLOBALS['cfg']['AjaxEnable'] ? ' class="ajax" ' : '' ); ?>
			value="<?php echo $caption_output; ?>"
			<?php echo $title_output . $onclick; ?> />
	</form>
</td>
<?php
}
 ...
 *
 * @param integer $pos_next
 *            the offset for the "next" page
 * @param integer $pos_prev
 *            the offset for the "previous" page
 * @param string $sql_query
 *            the URL-encoded query
 * @param string $id_for_direction_dropdown
 *            the id for the direction dropdown
 *            
 * @return nothing
 *
 * @global string $db the database name
...
<table border="0" cellpadding="0" cellspacing="0" class="navigation">
   ...
   <td>
    ...
    <td>
            ...
            <input type="hidden" name="sql_query"
					value="<?php echo $html_sql_query; ?>" /> <input type="hidden"
					name="pos" value="0" /> <input type="hidden"
					name="session_max_rows" value="all" /> <input type="hidden"
					name="goto" value="<?php echo $goto; ?>" /> <input type="submit"
					name="navig" value="<?php echo __('Show all'); ?>" />
			</form>
		</td>
        <?php
    } // end show all
    ...
    <td>
		...
		<td>
			<div class="restore_column hide">
				<input type="submit"
					value="<?php echo __('Restore column order'); ?>" />
				<div class="navigation_separator">|</div>
			</div>
		</td>
...
// if displaying a VIEW, $unlim_num_rows could be zero because
    ...
    <td class="navigation_goto">
            ...
            <input type="text" name="pos" size="3"
					value="<?php echo (($pos_next >= $unlim_num_rows) ? 0 : $pos_next); ?>"
					class="textfield" onfocus="this.select()" />
            <?php echo __('Number of rows') . ': ' . "\n"; ?>
            <input type="text" name="session_max_rows" size="3"
					value="<?php echo (($_SESSION['tmp_user_values']['max_rows'] != 'all') ? $_SESSION['tmp_user_values']['max_rows'] : $GLOBALS['cfg']['MaxRows']); ?>"
					class="textfield" onfocus="this.select()" />
        <?php
 ...
 *
 * @param
 *            array &$is_display which elements to display
 * @param
 *            array &$fields_meta the list of fields properties
 ...
 *            sort expression
 * @param string $sort_expression_nodirection
 *            sort expression without direction
...
<tr>
	<th
		colspan="<?php echo $num_rows + floor($num_rows/$_SESSION['tmp_user_values']['repeat_cells']) + 1; ?>"></th>
</tr>
<?php
        } // end vertical mode
    }    
    ...
    for ($j = 0; $j < $fields_cnt; $j ++) {
        ...
        if ($is_display['sort_lnk'] == '1') {
            ...
            if (! $is_in_sort) {
                ...
                $sort_order .= ' ASC';
                $order_img = ' ' . PMA_getImage('s_desc.png', __('Descending'), array(
                    'class' => "soimg$i",
                    'title' => ''
                ));
                $order_img .= ' ' . PMA_getImage('s_asc.png', __('Ascending'), array(
                    'class' => "soimg$i hide",
                    'title' => ''
                ));
            } else {
 ...
 * @param string $class
 *            class of table cell
 * @param bool $condition_field
 *            whether to add CSS class condition
 * @param string $value
 *            value to display
 *            
 ...
 *            whether to add CSS class condition
 * @param object $meta
 *            the meta-information about this field
 * @param string $align
 *            cell allignment
 *            
 * @return string the td
...
function PMA_addClass($class, $condition_field, $meta, $nowrap, $is_field_truncated = false, $transform_function = '', $default_function = '')
{
    ...
    $result = $class . ($condition_field ? ' condition' : '') . $nowrap . ' ' . ($is_field_truncated ? ' truncated' : '') . ($transform_function != $default_function ? ' transformed' : '') . $enum_class . $set_class . $bit_class . $mime_type_class;
 ...
 *
 * @param
 *            integer &$dt_result the link id associated to the query which results have
 *            to be displayed
 * @param
 *            array &$is_display which elements to display
 * @param array $map
    ...
    while ($row = PMA_DBI_fetch_row($dt_result)) {
        ...
        if ($is_display['edit_lnk'] != 'nn' || $is_display['del_lnk'] != 'nn') {
            ...
            if ($is_display['edit_lnk'] == 'ur') { // update row case
                ...
                $edit_url = 'tbl_change.php' . PMA_generate_common_url($_url_params + array(
                    'default_action' => 'update'
                ));
                $copy_url = 'tbl_change.php' . PMA_generate_common_url($_url_params + array(
            ...
            if (($GLOBALS['cfg']['RowActionLinks'] == 'left' || $GLOBALS['cfg']['RowActionLinks'] == 'both') && ($_SESSION['tmp_user_values']['disp_direction'] == 'horizontal' || $_SESSION['tmp_user_values']['disp_direction'] == 'horizontalflipped')) {
                if (! isset($js_conf)) {
                    $js_conf = '';
                }
            ...
            if ($meta->numeric == 1) {
                ...
                if (! isset($row[$i]) || is_null($row[$i])) {
                    ...
                    if ('GEOM' == $_SESSION['tmp_user_values']['geometry_display']) {
                        ...
                        if (PMA_strlen($wktval) > $GLOBALS['cfg']['LimitChars'] && $_SESSION['tmp_user_values']['display_text'] == 'P') {
                            $wktval = PMA_substr($wktval, 0, $GLOBALS['cfg']['LimitChars']) . '...';
                            $is_field_truncated = true;
                        }
                        
                        $vertical_display['data'][$row_no][$i] = '<td ' . PMA_prepare_row_data($class, $condition_field, $analyzed_sql, $meta, $map, $wktval, $transform_function, $default_function, '', $where_comparison, $transform_options, $is_field_truncated);
                        ...
                        if ($_SESSION['tmp_user_values']['display_binary']) {
                            ...
                            if (PMA_strlen($wkbval) > $GLOBALS['cfg']['LimitChars'] && $_SESSION['tmp_user_values']['display_text'] == 'P') {
                                $wkbval = PMA_substr($wkbval, 0, $GLOBALS['cfg']['LimitChars']) . '...';
                                $is_field_truncated = true;
                            }
                            
                            $vertical_display['data'][$row_no][$i] = '<td ' . PMA_prepare_row_data($class, $condition_field, $analyzed_sql, $meta, $map, $wkbval, $transform_function, $default_function, '', $where_comparison, $transform_options, $is_field_truncated);
                        } else {
                            $wkbval = PMA_handle_non_printable_contents('BINARY', $row[$i], $transform_function, $transform_options, $default_function, $meta, $_url_params);
        ...
        if (($GLOBALS['cfg']['RowActionLinks'] == 'right' || $GLOBALS['cfg']['RowActionLinks'] == 'both') && ($_SESSION['tmp_user_values']['disp_direction'] == 'horizontal' || $_SESSION['tmp_user_values']['disp_direction'] == 'horizontalflipped')) {
            if (! isset($js_conf)) {
                $js_conf = '';
            }
...
function PMA_displayTable_checkConfigParams()
{
    ...
    if (PMA_isValid($_REQUEST['disp_direction'], array(
        'horizontal',
        'vertical',
        'horizontalflipped'
    ))) {
    ...
    if ((PMA_isValid($_REQUEST['session_max_rows'], 'numeric') && (int) $_REQUEST['session_max_rows'] == $_REQUEST['session_max_rows']) || $_REQUEST['session_max_rows'] == 'all') {
    ...
    if (PMA_isValid($_REQUEST['display_text'], array(
        'P',
        'F'
    ))) {
    ...
    if (PMA_isValid($_REQUEST['relational_display'], array(
        'K',
        'D'
    ))) {
    ...
    if (PMA_isValid($_REQUEST['geometry_display'], array(
        'WKT',
        'WKB',
        'GEOM'
    ))) {
 ...
 *            to be displayed
 * @param
 *            array &$the_disp_mode the display mode
 * @param array $analyzed_sql
 *            the analyzed query
 *            
 * @global string $db the database name
...
<?php
    ...
    if ($is_display['del_lnk'] == 'dr' && $is_display['del_lnk'] != 'kp') {
        ...
        if ($_SESSION['tmp_user_values']['disp_direction'] != 'vertical') {
            echo '<img class="selectallarrow" width="38" height="22"' . ' src="' . $GLOBALS['pmaThemeImage'] . 'arrow_' . $GLOBALS['text_dir'] . '.png' . '"' . ' alt="' . __('With selected:') . '" />';
        }
...
function PMA_displayResultsOperations($the_disp_mode, $analyzed_sql)
{
    ...
    if (! PMA_DRIZZLE && ! isset($analyzed_sql[0]['queryflags']['procedure'])) {
        echo PMA_linkOrButton('view_create.php' . $url_query, PMA_getIcon('b_views.png', __('Create view'), true), '', true, true, '') . "\n";
    }
    if ($header_shown) {
        echo '</fieldset><br />';
    }
 ...
 *            parameters that should go to the download link
 *            
 ...
 *
 * @param string $class
 *            css classes for the td element
 * @param bool $condition_field
 *            whether the column is a part of the where clause
 ...
 *            default function
 * @param string $nowrap
 *            'nowrap' if the content should not be wrapped
 * @param string $where_comparison
 *            data for the where cluase
 * @param array $transform_options
 *            array of options for transformation
 * @param bool $is_field_truncated
 *            whether the field is truncated
 *            
    ...
    if (isset($map[$meta->name])) {
        ...
        if (isset($GLOBALS['printview']) && $GLOBALS['printview'] == '1') {
            ...
            $_url_params = array(
                ...
                'pos' => '0',
                'sql_query' => 'SELECT * FROM ' . PMA_backquote($map[$meta->name][3]) . '.' . PMA_backquote($map[$meta->name][0]) . ' WHERE ' . PMA_backquote($map[$meta->name][1]) . $where_comparison
            );
 ...
 *            array with explicit indexes for all the display elements
 * @param string $row_no
 *            the row number
 * @param string $where_clause_html
 *            url encoded where cluase
 * @param array $condition_array
 *            array of conditions in the where cluase
...
function PMA_generateCheckboxForMulti($del_url, $is_display, $row_no, $where_clause_html, $condition_array, $del_query, $id_suffix, $class)
{
    ...
    if (! empty($del_url) && $is_display['del_lnk'] != 'kp') {
        ...
        $ret .= ' align="center">' . '<input type="checkbox" id="id_rows_to_delete' . $row_no . $id_suffix . '" name="rows_to_delete[' . $where_clause_html . ']"' . ' class="multi_checkbox"' . ' value="' . htmlspecialchars($del_query) . '" ' . (isset($GLOBALS['checkall']) ? 'checked="checked"' : '') . ' />' . '<input type="hidden" class="condition_array" value="' . htmlspecialchars(json_encode($condition_array)) . '" />' . '    </td>';
    }
    return $ret;
}
 ...
 *            edit url
 * @param string $class
 *            css classes for td element
 * @param string $edit_str
 *            text for the edit link
...
function PMA_generateEditLink($edit_url, $class, $edit_str, $where_clause, $where_clause_html)
{
    $ret = '';
    if (! empty($edit_url)) {
        $ret .= '<td class="' . $class . '" align="center" ' . ' ><span class="nowrap">' . PMA_linkOrButton($edit_url, $edit_str, array(), false);
        /*
...
function PMA_generateCopyLink($copy_url, $copy_str, $where_clause, $where_clause_html, $class)
{
    ...
    if (! empty($copy_url)) {
        ...
        $ret .= 'align="center" ' . ' ><span class="nowrap">' . PMA_linkOrButton($copy_url, $copy_str, array(), false);
        /*
         * Where clause for selecting this row uniquely is provided as
         * a hidden input. Used by jQuery scripts for handling grid editing
         */
 ...
 *            the position of the checkbox and links
 * @param string $del_url
 *            delete url
 * @param array $is_display
 *            array with explicit indexes for all the display elements
 ...
 *            copy url
 * @param string $class
 *            css classes for the td elements
 * @param string $edit_str
 *            text for the edit link
 * @param string $copy_str
 *            text for the copy link
