...
if (strlen($db) && (! empty($db_rename) || ! empty($db_copy))) {
    ...
    /**
     * Database has been successfully renamed/moved.
...
<div class="operations_half_width">
	<form id="rename_db_form"
        ...
        ?>
    <input type="hidden" name="what" value="data" /> <input
			type="hidden" name="db_rename" value="true" />
    <?php echo PMA_generate_common_hidden_inputs($db); ?>
    <fieldset>
			<legend>
    <?php
		...
		<fieldset class="tblFooters">
			<input id="rename_db_input" type="submit"
...
<?php
    ...
    if (($is_superuser || $GLOBALS['cfg']['AllowUserDropDatabase']) && ! $db_is_information_schema && (PMA_DRIZZLE || $db != 'mysql')) {
        ?>
<div class="operations_half_width">
...
<?php
        ...
        <li><a
				...
				<?php echo ($GLOBALS['cfg']['AjaxEnable'] ? 'id="drop_db_anchor"' : ''); ?>>
...
<div class="operations_half_width clearfloat">
    ...
    ?>
			...
			<input type="text" name="newname" size="30" class="textfield"
				value="" /><br />
<?php
    $choices = array(
    ...
    ?>
        <input type="checkbox" name="create_database_before_copying"
			...
			<input type="checkbox" name="drop_if_exists" value="true"
				id="checkbox_drop" /> <label for="checkbox_drop"><?php echo sprintf(__('Add %s'), $drop_clause); ?></label><br />
			<input type="checkbox" name="sql_auto_increment" value="1"
				checked="checked" id="checkbox_auto_increment" /> <label
				for="checkbox_auto_increment">
            <?php echo __('Add AUTO_INCREMENT value'); ?></label><br />
			<input type="checkbox" name="add_constraints" value="1"
				id="checkbox_constraints" /> <label for="checkbox_constraints">
