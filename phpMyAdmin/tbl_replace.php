...
if (isset($_REQUEST['after_insert']) && in_array($_REQUEST['after_insert'], array(
    'new_insert',
    'same_insert',
    'edit_next'
))) {
    ...
    if (isset($_REQUEST['where_clause'])) {
        if ($_REQUEST['after_insert'] == 'same_insert') {
            ...
            foreach ($_REQUEST['where_clause'] as $one_where_clause) {
                $local_query = 'SELECT * FROM ' . PMA_backquote($GLOBALS['db']) . '.' . PMA_backquote($GLOBALS['table']) . ' WHERE ' . str_replace('` =', '` >', $one_where_clause) . ' LIMIT 1;';
...
if (isset($_REQUEST['where_clause'])) {
    ...
    $loop_array = (is_array($_REQUEST['where_clause']) ? $_REQUEST['where_clause'] : array(
...
foreach ($loop_array as $rownumber => $where_clause) {
    ...
    foreach ($me_fields_name as $key => $field_name) {
        ...
        if (empty($me_funcs[$key])) {
            ...
            $cur_value = "'" . $uuid . "'";
        } elseif ((in_array($me_funcs[$key], $gis_from_text_functions) && substr($val, 0, 3) == "'''") || in_array($me_funcs[$key], $gis_from_wkb_functions)) {
            // Remove enclosing apostrophes
            $val = substr($val, 1, strlen($val) - 2);
            // Remove escaping apostrophes
            $val = str_replace("''", "'", $val);
            $cur_value = $me_funcs[$key] . '(' . $val . ')';
        } elseif (! in_array($me_funcs[$key], $func_no_param) || ($val != "''" && in_array($me_funcs[$key], $func_optional_param))) {
            $cur_value = $me_funcs[$key] . '(' . $val . ')';
        } else {
    ...
    if (count($query_values) > 0) {
        if ($is_insert) {
            ...
            // build update query
            $query[] = 'UPDATE ' . PMA_backquote($GLOBALS['db']) . '.' . PMA_backquote($GLOBALS['table']) . ' SET ' . implode(', ', $query_values) . ' WHERE ' . $where_clause . ($_REQUEST['clause_is_unique'] ? '' : ' LIMIT 1');
        }
    }
...
if ($is_insert && count($value_sets) > 0) {
    ...
    $query[] = $insert_command . 'INTO ' . PMA_backquote($GLOBALS['db']) . '.' . PMA_backquote($GLOBALS['table']) . ' (' . implode(', ', $query_fields) . ') VALUES (' . implode('), (', $value_sets) . ')';
...
if ($GLOBALS['is_ajax_request'] == true) {
     ...
     * transformed fields, if they were edited.
     * After that, output the correct
        ...
        foreach ($rel_fields as $cell_index => $curr_cell_rel_field) {
            ...
            foreach ($curr_cell_rel_field as $rel_field => $rel_field_value) {
                ...
                $_url_params = array(
                    ...
                    'pos' => '0',
                    'sql_query' => 'SELECT * FROM ' . PMA_backquote($map[$rel_field]['foreign_db']) . '.' . PMA_backquote($map[$rel_field]['foreign_table']) . ' WHERE ' . PMA_backquote($map[$rel_field]['foreign_field']) . $where_comparison
                );
